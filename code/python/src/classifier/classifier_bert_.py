...
def fit_bert_holdout(df_all: pd.DataFrame, split_at_row: int,
    ...
    cutil.save_scores(predictions_text_labels, true_text_labels, model_name, target_and_feature,
...
# this method is almost identical to fit_bert_holdout, except it saves the model upon completion and not evalute it.
def fit_bert_trainonly(df_all: pd.DataFrame, split_at_row: int,  # indicate train size
                       param_label_field: int,
                       param_sentence_length: int,
                       param_batch_size: int,
                       param_training_epoch: int,
    ...
    # saving model
    ############################
    torch.save(model, outfolder + "/classifier")
    with open(outfolder + "/labelencoder.pk", 'wb') as handle:
